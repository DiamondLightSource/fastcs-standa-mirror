...
class MirrorAttributeIO(AttributeIO[float, MirrorAttributeIORef]):
    ...
    async def update(self, attr: AttrR[float, MirrorAttributeIORef]):
        ...
        if attr.io_ref.name == "speed":
            move_settings = self._master.pitch.motor.get_move_settings()
    ...
    async def send(
        self, attr: AttrW[NumberT, MirrorAttributeIORef], value: NumberT
    ) -> None:
        ...
        if attr.io_ref.name == "speed":
            ...
            self._master.pitch.motor.set_move_settings(move_settings_pitch)
